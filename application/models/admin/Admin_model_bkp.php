...
class Admin_model extends CI_model
{
	...
	public function get_own_vehicle_metrics()
	{
		...
		$this->db->select_sum('exact_km');
		$this->db->select_sum('transport_percent');
		$this->db->select_sum('diesel_expense');
		$this->db->select_sum('market_freight');
		$this->db->select_sum('invoice_value');

		$row = $this->db->get()->row();
		$diesel_expense = $row->diesel_expense ?? 0;
		$driver_expense = ($diesel_expense) ?? 0;
		$invoice_val = $row->invoice_value ?? 0;
		$transport_percent = $invoice_val > 0 ? (($driver_expense + $diesel_expense) * 100) / $invoice_val : 0;
		return [
			'exact_km' => $row->exact_km ?? 0,
			'transport_percent' => $transport_percent,
			'diesel_expense' => $row->diesel_expense ?? 0,
			'market_freight' => $row->market_freight ?? 0,
			'invoice_count' => $row->invoice_value ?? 0,
		];
	}
	public function get_other_rm_mb_rejection_data($machine_id)
	{
		...
		$this->db->select_sum('tbl_production_schedules_rm_details.return_stock_qty', 'returned_qty');
		$this->db->from('tbl_production_schedules_rm_details');
		$this->db->join('tbl_production_schedules','tbl_production_schedules_rm_details.schedule_id = tbl_production_schedules.id','left');
		if (!empty($machine_id)) {
			$this->db->where('tbl_production_schedules.machine_id', $machine_id);
			$this->db->where('tbl_production_schedules.plant_id', $machine_res->plant_id);
		}
		if (!empty($raw_material)) {
			$this->db->where('tbl_production_schedules_rm_details.item_table_id', $raw_material);
		}
		$this->db->where('tbl_production_schedules_rm_details.item_type', '1');
		$returned_rm = $this->db->get()->row()->returned_qty ?? 0;
		// echo"<pre>";print_r($returned_rm);exit;
		$this->db->reset_query();
		// if(!empty($rm)){
		// 	$rm = $rm - $returned_rm;
		// }
		...
		if ($date_range) {
			...
			if (count($dates) == 2) {
				...
				if ($start && $end) {
					$this->db->where('tbl_production_schedules_rm_details.created_on >=', $start);
					$this->db->where('tbl_production_schedules_rm_details.created_on <=', $end);
				}
			}else if (count($dates) == 1) {
				$single_date = trim($dates[0]);
				$single = date('Y-m-d', strtotime(str_replace('/', '-', $single_date)));
				...
				if ($single) {
					$this->db->where('DATE(tbl_production_schedules_rm_details.created_on)', $single);
				}
			}
		}
		$this->db->where('tbl_production_schedules_rm_details.item_type', '2');
		$returned_mb = $this->db->get()->row()->returned_qty ?? 0;
		// if(!empty($mb)){
		// 	$mb = $mb - $returned_mb;
		// }
	...
	public function get_dashboard_metrics()
	{
		...
		if ($date_range) {
			...
			if (count($dates) == 2) {
				...
				if ($start && $end) {
					$this->db->where('DATE(tbl_production_schedules_rm_details.created_on) >=', $start);
					$this->db->where('DATE(tbl_production_schedules_rm_details.created_on) <=', $end);
				}
			}else if (count($dates) == 1) {
				$single_date = trim($dates[0]);
				$single = date('Y-m-d', strtotime(str_replace('/', '-', $single_date)));
		...
		if (!empty($master_batch_id)) {
			$this->db->where('tbl_production_schedules_rm_details.item_table_id', $master_batch_id);
		}
		if ($date_range) {
			$dates = explode(' to ', str_replace(' - ', ' to ', $date_range));
			if (count($dates) == 2) {
		...
		if (!empty($raw_material)) {
			$this->db->where('tbl_rejection_article_list_production_details.raw_material_id', $raw_material);
		}
		if (!empty($machine)) {
		...
		if (!empty($date_range)) {
			...
			if (count($dates) === 2) {
				...
				if ($single) {
					$this->db->where('DATE(tbl_maintenance_production.created_on)', $single);
				}
			}
    	}
		if (!empty($machine)) {
		...
		foreach ($brands as $brand) {
			// Get the latest order for this brand
			$this->db->select_max('created_on');
			$this->db->where('brand_type_id', $brand->id);
			$this->db->where('is_deleted', '0');
			$latest_order = $this->db->get('tbl_order_sub_details')->row();

			if (empty($latest_order->created_on)) {
				$inactive_count++;
			} else {
				$last_order_date = $latest_order->created_on;
				$diff = (strtotime($today) - strtotime($last_order_date)) / (60 * 60 * 24);
				if ($diff > $days) {
					$inactive_count++;
				}
			}
		...
		foreach ($all_assign_tasks as $task) {
			$this->db->select('updated_on,created_on');
			$this->db->where('task_id', $task->task_id);
			$this->db->where('last_updated_by', $task->assign_to_id);
			$this->db->where('assign_to_id !=', $task->assign_to_id);
			$this->db->order_by('updated_on', 'ASC');
			$completed = $this->db->get('tbl_manual_task_history')->row();
	...
	public function get_store_dashboard_metrics()
	{
		...
		if ($this->session->userdata('is_admin') != '1') {
			$this->db->where('(my_plant_id <> ' . $this->db->escape($this->session->userdata('assign_plant_id')) . ' OR plant_id IS NULL)', NULL, FALSE);
		}
	...
	public function get_all_machines_production()
	{
		...
		$this->db->where('is_deleted', '0');
		if ($this->session->userdata('is_admin') != '1') {
			$this->db->where('plant_id', $this->session->userdata('assign_plant_id'));
		}
	...
	public function get_all_article_level_analysis()
	{
		...
		if ($this->session->userdata('is_admin') != '1') {
			$this->db->where('tbl_article_production_details.plant_id', $this->session->userdata('assign_plant_id'));
		}
		if (!empty($machine)) {
		...
		if ($date_range) {
			...
			if (count($dates) == 2) {
				...
				if ($start && $end) {
					$this->db->where('tbl_article_production_details.created_on >=', $start);
					$this->db->where('tbl_article_production_details.created_on <=', $end);
				}
			}
		}
		if ($article) {
	...
	public function get_machine_production_month()
	{
		...
		$this->db->select('tbl_article_production_details.machine_id, tbl_article_production_details.created_on, tbl_article_production_details.approved_qty, tbl_machine_master.machine_name');
		$this->db->from('tbl_article_production_details');
		...
		foreach ($query as $row) {
			$month = date('n', strtotime($row['created_on'])); // Extract month (1-12)
	...
	public function get_prouction_planed_actual()
{
	...
	public function get_single_bom()
	{
		$this->db->where('article_id', $this->uri->segment(2));
		$this->db->where('is_deleted', '0');
		$result = $this->db->get('tbl_particulars_bom');
		return $result->result();
	}
	...
	public function set_production_bom()
	{
		...
		if ($count == count($sub_categories) && $count == count($uoms) && $count == count($quantities)) {
			$this->db->where('bom_id', $this->uri->segment(3));
			$this->db->delete('tbl_particulars_bom');
			for ($i = 0; $i < $count; $i++) {
				if ($sub_categories[$i] == '' || $uoms[$i] == '' || $quantities[$i] == '') {
					continue;
				}
				$data = array(
					'particulars_id' => $particular_ids[$i],
					'article_id' => $this->uri->segment(2),
					'sub_category_id' => $sub_categories[$i],
					'uom_id' => $uoms[$i],
					'quantity' => $quantities[$i],
					'bom_id' => $production_bom_id,
					'created_on' => date('Y-m-d H:i:s'),
				);
				$this->db->insert('tbl_particulars_bom', $data);
			}
			return '1';
		}
	}
	...
	public function set_task()
	{
		...
		$this->db->where('id', $this->input->post('assign_to'));
		...
		$notification_data = array(
			'notification_title' => $title,
			'notification_description' => $description,
			'notification_department' => $this->input->post('assign_department'),
			'landing_page' => $landing_page,
			'order_id' => $task_id,
		...
		$this->send_task_notification_by_token($this->input->post('assign_to'), $title, $description, $landing_page, $notification_according, $user_data->plant_id);
			...
			foreach ($article_ids as $key => $article_id) {
				...
				if ($dispatch_qty > 0) {
					...
					if ($dispatch_qty > 0) {
						...
						if ($stock_dispatch) {
							$total_qty = $stock_dispatch->total_quantity - $dispatch_qty;
							$update_data = array(
								'total_quantity' => $total_qty,
								'updated_on' => date('Y-m-d H:i:s'),
							);
	...
	{
		$this->db->select('user_data.*,tbl_krivisha_department.department,tbl_plant_master.plant_name');
		$this->db->from('user_data');
		$this->db->join('tbl_krivisha_department', 'user_data.department_id = tbl_krivisha_department.id', 'left');
		$this->db->join('tbl_plant_master', 'user_data.plant_id = tbl_plant_master.id', 'left');
	...
	public function set_new_option()
	{
		...
		} else if ($this->input->post('master_type') == 'air_pin') {
			$this->db->where('is_deleted', '0');
			$this->db->where('air_pin', $this->input->post('new_option'));
			$result = $this->db->get('tbl_air_pin');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"air_pin" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_air_pin', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'spring') {
			$this->db->where('is_deleted', '0');
			$this->db->where('spring', $this->input->post('new_option'));
			$result = $this->db->get('tbl_spring_master');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"spring" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_spring_master', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'pu_nipples') {
			$this->db->where('is_deleted', '0');
			$this->db->where('pu_nipples', $this->input->post('new_option'));
			$result = $this->db->get('tbl_pu_nipples_master');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"pu_nipples" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_pu_nipples_master', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'ejector_pin') {
			$this->db->where('is_deleted', '0');
			$this->db->where('ejector_pin', $this->input->post('new_option'));
			$result = $this->db->get('tbl_ejector_pin_master');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"ejector_pin" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_ejector_pin_master', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'i_bolt') {
			$this->db->where('is_deleted', '0');
			$this->db->where('i_bolt', $this->input->post('new_option'));
			$result = $this->db->get('tbl_i_bolt');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"i_bolt" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_i_bolt', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'cord') {
			$this->db->where('is_deleted', '0');
			$this->db->where('cord', $this->input->post('new_option'));
			$result = $this->db->get('tbl_cord');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"cord" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_cord', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'o_ring') {
			$this->db->where('is_deleted', '0');
			$this->db->where('o_ring', $this->input->post('new_option'));
			$result = $this->db->get('tbl_o_ring');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"o_ring" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_o_ring', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'insert_slot_plate') {
			$this->db->where('is_deleted', '0');
			$this->db->where('insert_slot_plate', $this->input->post('new_option'));
			$result = $this->db->get('tbl_insert_slot_plate');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"insert_slot_plate" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_insert_slot_plate', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'core_cylinder_seal') {
			$this->db->where('is_deleted', '0');
			$this->db->where('core_cylinder_seal', $this->input->post('new_option'));
			$result = $this->db->get('tbl_core_cylinder_seal');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"core_cylinder_seal" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_core_cylinder_seal', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'seal') {
			$this->db->where('is_deleted', '0');
			$this->db->where('seal', $this->input->post('new_option'));
			$result = $this->db->get('tbl_seal');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"seal" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_seal', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'hose_pipe') {
			$this->db->where('is_deleted', '0');
			$this->db->where('hose_pipe', $this->input->post('new_option'));
			$result = $this->db->get('tbl_hose_pipe');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"hose_pipe" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_hose_pipe', $data);
				echo 0;
			} else {
				echo 1;
			}
		} else if ($this->input->post('master_type') == 'alankey_bolt') {
			$this->db->where('is_deleted', '0');
			$this->db->where('alankey_bolt', $this->input->post('new_option'));
			$result = $this->db->get('tbl_alankey_bolt');
			$result = $result->row();
			if (empty($result)) {
				$data = array(
					"alankey_bolt" => $this->input->post('new_option'),
					"created_on" => date('Y-m-d H:i:s')
				);
				$this->db->insert('tbl_alankey_bolt', $data);
				echo 0;
			} else {
				echo 1;
			}
	...
	{
		$this->db->where('is_deleted', '0');
		$this->db->where('brand_name', $this->input->post('brand_name'));
		if ($this->input->post('id') != "0") {
			$this->db->where('id !=', $this->input->post('id'));
		}
		$result = $this->db->get('tbl_brand_master')->row();
		if (!empty($result)) {
			echo '1';
		} else {
			echo "0";
	...
	public function get_all_outward_order_list($length, $start, $search)
	{
		...
		$this->db->order_by('tbl_auto_task_list.created_on', 'DESC');
		$count_query = $this->db->get();
		$total_count = $count_query->row()->total_count ?? 0;
	...
	public function get_employees_by_department()
	{
		...
		$this->db->where('user_data.is_deleted', '0');
		$this->db->where('user_data.department_id', $department_id);
	...
	public function get_sub_order_task_details()
	{
		...
		if ($order_type == 'Create Order') {
			...
			$this->db->group_by('tbl_production_schedules.id'); // ✅ This is important
			$result = $this->db->get();
		}
		if ($result && $result->num_rows() > 0) {
			echo json_encode($result->result_array());
	...
	{
		$order_id = $this->input->post('dispatch_id');
		...
		$this->db->where('tbl_dispatch_order_data.dispatch_id', $order_id);
	...
	public function get_all_manual_task_list($length, $start, $search)
	{
		...
		} else if ($this->session->userdata('assign_department_id') != '25') {
			$this->db->where('tbl_manual_task.department_id', $this->session->userdata('assign_department_id'));
		}
	...
	public function get_all_material_qty_request_list($length, $start, $search, $according_plant)
	{
		...
		if ($this->session->userdata('is_admin') != '1') {
			if ($according_plant == '1') {
				// $this->db->where('(tbl_rm_request_qty.my_plant_id <> ' . $this->db->escape($this->session->userdata('assign_plant_id')) . ' OR tbl_rm_request_qty.plant_id IS NULL)', NULL, FALSE);
	...
	public function set_production_report()
	{
		...
		$data = [
			'production_date' => date("Y-m-d", strtotime($this->input->post('production_date'))),
			'machine_id' => $this->input->post('machine_id'),
	...
	public function get_all_production_report_list_count($search)
	{
	...
	public function set_maintenance_list()
	{
		...
		if ($status_of_work == '1' && $approve_status == '0') {
			if ($mwo_record->maintaince == '1' || $mwo_record->maintaince == '3') {
	...
	public function set_own_vehicle_details()
	{
		...
		$data = array(
			...
			'in_km' => !empty($out_km) ? $out_km : ($in_km ?: 0),
			'out_km' => !empty($in_km) ? $in_km : ($out_km ?: 0),
	...
	{
		$values = implode(',', $this->input->post('link'));
		$data = array(
			'staff_id' => $this->input->post('employee'),
			'previleges' => $values,
		);
		...
		if (empty($result)) {
			...
			$new_arr = array_merge($data, $date);
			$this->db->insert('tbl_assign_previlege', $new_arr);
			return 1;
		} else {
			$this->db->where('staff_id', $this->uri->segment(2));
	...
	public function get_outward_order_status($order_id)
	{
		...
		if ($orders) {
			foreach ($orders as $order) {
				...
				if ($order->order_department_status == 3) {
					$dispatch_count += 1;
				}
				if ($order->order_department_status == 2) {
					$printing_count += 1;
				}
	...
	public function get_total_stock_list($length, $start, $search, $stock_type)
	{
		if ($stock_type == 'raw_material') {
			...
			if ($this->session->userdata('assign_department_id') != '25') {
				$this->db->where('tbl_master_batch_stock_report.plant_id', $this->session->userdata('assign_plant_id'));
			}
			if ($this->input->post('plant_id') != "") {
	...
	public function get_fast_and_over_moving_list()
	{
		...
		// print_r($fast_moving);
		// ✅ OVER-PURCHASED (Purchased but NOT used)
		$this->db->select('tbl_rm_master.rm_name, SUM(tbl_inward_order_data.inward_quantity) as total_purchased');
		$this->db->from('tbl_inward_order_data');
		$this->db->join('tbl_rm_master', 'tbl_rm_master.id = tbl_inward_order_data.raw_material_id', 'left');
		$this->db->where('tbl_inward_order_data.is_deleted', '0');
		$this->db->where('tbl_inward_order_data.created_on >=', $start.' 00:00:00');
		...
		$this->db->group_by('tbl_rm_master.rm_name');
        ...
        LEFT JOIN ($used_data) AS u ON p.rm_name = (
            SELECT rm_name FROM tbl_rm_master WHERE tbl_rm_master.id = u.raw_material_id LIMIT 1
        )
		...
		if (!empty($date)) {
			...
			if (strpos($date, ' to ') !== false) {
				...
				if (count($dates) == 2) {
					...
					$this->db->where('tbl_inward_order_data.created_on >=', $start_date);
					$this->db->where('tbl_inward_order_data.created_on <=', $end_date);
				}
		...
		if (!empty($date)) {
			if (strpos($date, ' to ') !== false) {
				...
				if (count($dates) == 2) {
					...
					$this->db->where('tbl_production_schedules.date >=', $start_date);
					$this->db->where('tbl_production_schedules.date <=', $end_date);
				}
			} else {
				$single_date = date('Y-m-d', strtotime($date));
	...
	public function get_machine_wise_requirement_store_dashboard($plant_id, $date, $machine_id)
	{
		...
		// 🔹 FINAL OUTPUT FORMATTING (3 DECIMALS ONLY)
