...
class Api_model extends CI_model
{
	...
	public function get_app_login()
	{
		...
		$this->db->where('org_password', $request['password']);
	...
	public function set_article_production_details_api()
	{
		...
		$allWeights = array_map('floatval', $weight_data);
		$data['average_qty'] = count($allWeights) > 0
			? array_sum($allWeights) / count($allWeights)
			: 0;
		...
		$data['status'] = $input['status'] ?? '';
		$data['plant_id'] = $input['plant_id'] ?? null;
		...
		foreach ($result as $key => $order) {
			...
			if ($check_sub->num_rows() > 0) {
				...
				foreach ($res as $key => $sub_order) {
					$this->db->select('dispatch_quantity,remaining_quantity');
					...
					if ($dispatch_result) {
						$sub_order->total_dispatch_quantity = $dispatch_result->dispatch_quantity;
						$sub_order->total_remaining_quantity = $dispatch_result->remaining_quantity;
					} else {
						$sub_order->total_dispatch_quantity = 0;
						$sub_order->total_remaining_quantity = 0;
					}
				}
	...
	public function set_requested_raw_material_api()
	{
		...
		if (!empty($request['sub_details']) && is_array($request['sub_details'])) {
			foreach ($request['sub_details'] as $sub) {
				...
				if ($raw_material_id && $request_quantity > 0) {
					$request_data = array(
						'request_no' => $request_no,
	...
	public function get_all_material_qty_request_list_api()
	{
		...
		if ($result) {
			foreach ($result as $key => $val) {
				$request_id = $val->id;
				$this->db->select('tbl_request_rm_qty_data.*, tbl_rm_master.rm_name, tbl_uom_master.uom_name, user_data.first_name AS approved_by_name');
