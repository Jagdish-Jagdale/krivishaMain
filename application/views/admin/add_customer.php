                ...
                <div class="container">
                    <form method="post" name="add_customer" id="add_customer" enctype="multipart/form-data">
                        <div class="row flex_wrap">
                            ...
                            <!-- 4 Sales Layer Hierarchy -->
                            <div class="form-group col-md-4 col-sm-6 col-xs-12">
                                <label>DG (Deputy General Manager)</label>
                                <select name="dg_id" id="dg_id" class="form-control js-example-basic-multiple">
                                    <option value="">Select DG</option>
                                    <?php if (!empty($employee)) {
                                        foreach ($employee as $emp) { ?>
                                            <option value="<?= $emp->id ?>" <?php if (!empty($single) && $single->dg_id == $emp->id) echo 'selected'; ?>><?= $emp->first_name ?></option>
                                    <?php }
                                    } ?>
                                </select>
                            </div>
                            <div class="form-group col-md-4 col-sm-6 col-xs-12">
                                <label>ASM (Area Sales Manager)</label>
                                <select name="asm_id" id="asm_id" class="form-control js-example-basic-multiple">
                                    <option value="">Select ASM</option>
                                    <?php if (!empty($employee)) {
                                        foreach ($employee as $emp) { ?>
                                            <option value="<?= $emp->id ?>" <?php if (!empty($single) && $single->asm_id == $emp->id) echo 'selected'; ?>><?= $emp->first_name ?></option>
                                    <?php }
                                    } ?>
                                </select>
                            </div>
                            <div class="form-group col-md-4 col-sm-6 col-xs-12">
                                ...
                                <select name="state_head_id" id="state_head_id" class="form-control js-example-basic-multiple">
                                    <option value="">Select State Head</option>
                                    <?php if (!empty($employee)) {
                                        foreach ($employee as $emp) { ?>
                                            <option value="<?= $emp->id ?>" <?php if (!empty($single) && $single->state_head_id == $emp->id) echo 'selected'; ?>><?= $emp->first_name ?></option>
                                    <?php }
                                    } ?>
                                </select>
                            </div>
                            <div class="form-group col-md-4 col-sm-6 col-xs-12">
                                ...
                                <select name="telecaller_id" id="telecaller_id" class="form-control js-example-basic-multiple">
                                    <option value="">Select Telecaller</option>
                                    <?php if (!empty($employee)) {
                                        foreach ($employee as $emp) { ?>
                                            <option value="<?= $emp->id ?>" <?php if (!empty($single) && $single->telecaller_id == $emp->id) echo 'selected'; ?>><?= $emp->first_name ?></option>
                                    <?php }
                                    } ?>
                                </select>
                            </div>
...
<script>
    $('#party_name, #gst_pan, #mobile').on('keyup', function(){
        check_unique_party_name();
    });
    function check_unique_party_name() {
        var party_name = $('#party_name').val();
        var gst_pan = $('#gst_pan').val();
        var mobile = $('#mobile').val();
        if(party_name != '' && gst_pan != '' && mobile != ''){
            $.ajax({
