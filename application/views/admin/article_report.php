...
<div class="right_col">
    ...
    <div class="main_page">
        <div class="page_body">
            ...
            <?php
            $used_rm_map = [];
            if (!empty($used_rm_rows)) {
                foreach ($used_rm_rows as $u) {
                    $used_rm_map[$u->raw_material_name] = (float)($u->used_qty ?? 0);
                }
            }
            $used_mb_map = [];
            if (!empty($used_mb_rows)) {
                foreach ($used_mb_rows as $u) {
                    $used_mb_map[$u->master_batch_name] = (float)($u->used_qty ?? 0);
                }
            }
            ?>
                <div class="share-grid">
                    ...
                    <div>
                        <table class="share-sheet">
                            <tr>
                                <th colspan="3">FORMULA RAW MATERIAL LIST - BALANCE MATERIAL<br><span style="font-weight:600;">(Only 1st 2 columns required)</span></th>
                            </tr>
                            <tr>
                                <th>Raw Material</th>
                            ...
                            if (!empty($balance_rows)) {
                                foreach ($balance_rows as $b) {
                                    if (!empty($b->raw_material_name)) {
                                        $has_rm_bal = true;
                                        $used_qty = $used_rm_map[$b->raw_material_name] ?? 0;
                ...
                <div class="share-grid">
                    ...
                    <div>
                        <table class="share-sheet">
                            <tr>
                                <th colspan="3">FORMULA MASTER BATCH - BALANCE MASTER BATCH<br><span style="font-weight:600;">(Only 1st 2 columns required)</span></th>
                            </tr>
                            <tr>
                                <th>Master Batch</th>
                                <th class="text-right">Consumed Qty</th>
                            ...
                            if (!empty($balance_rows)) {
                                foreach ($balance_rows as $b) {
                                    if (!empty($b->master_batch_name)) {
                                        $has_mb_bal = true;
                                        $used_qty = $used_mb_map[$b->master_batch_name] ?? 0;
                            ...
                            <tr>
                                ...
                                <td class="text-right">
                                    ...
                                    <?php if ($downtime_minutes > 0 && !empty($row->downtime_reasons)) {
                                        ...
                                        <div class="downtime-reasons">                                                <?php foreach ($reason_parts2 as $rp2) { ?>
                                                    <?php
                                                        $rp2_text = $rp2;
        ...
        function initProductionReport($) {
            ...
            function buildExportHtml() {
                ...
                $('.summary-grid:visible, .report-section:visible').each(function() {
                    ...
                    // Strip inline widths and styles that scramble the table columns
                ...
                return '<html><head><title>Production Report</title><style>' +
                    'body{font-family:Arial,sans-serif;padding:20px;color:#000;} ' +
                    'table{width:100%;border-collapse:collapse;margin-bottom:18px;} ' +
                    'th,td{border:1px solid #d1d9e6;padding:8px;vertical-align:middle;font-size:12px;color:#000;} ' +
                    'h2.section-head{font-size:15px;margin:20px 0 0;text-transform:uppercase;background:#0056d0;color:#fff;padding:10px 15px;font-weight:800;text-align:left;border-radius:4px 4px 0 0;} ' +
                    '.report-wrap{margin-bottom:20px;box-sizing:border-box;} ' +
                    '.text-right{text-align:right;} ' +
                    
                    '/* Summary Grid & Cards */' +
                    '.summary-grid{display:flex;flex-wrap:wrap;gap:12px;margin-bottom:18px;width:100%;}' +
                    '.summary-card{flex:1;min-width:200px;border:1px solid #d7dde5;border-radius:8px;background:linear-gradient(180deg, #ffffff 0%, #f8fbff 100%);padding:14px 16px;box-sizing:border-box;}' +
                    '.summary-card .label{display:block;font-size:11px;font-weight:700;text-transform:uppercase;color:#5a6472;margin-bottom:6px;}' +
                    '.summary-card .value{font-size:20px;font-weight:800;color:#11213f;}' +
                    '.summary-card .subvalue{margin-top:4px;color:#6b7280;font-size:11px;}' +
                    
                    '/* Shift Breakdown styling */' +
                    '.shift-breakdown .line{padding:2px 0;border-bottom:1px solid #222;}' +
                    '.shift-breakdown .line:last-child{border-bottom:0;}' +
                    '.shift-breakdown .total{margin-top:2px;padding-top:2px;border-top:1px solid #222;font-weight:700;}' +
                    
                    '/* Downtime Reasons styling */' +
                    '.downtime-reasons{margin-top:4px;text-align:left;font-size:10px;color:#c0392b;}' +
                    '.dt-reason-line{padding:1px 0;border-top:1px dashed #e0b9b9;line-height:1.3;}' +
                    '.dt-reason-line:first-child{border-top:none;}' +
                    
                    '/* Table styling */' +
                    '#overview_table{width:100% !important;table-layout:fixed;}' +
                    '#overview_table th, #overview_table td{padding:4px 5px !important;font-size:10px !important;white-space:normal !important;word-break:break-all;}' +
                    '#overview_table thead th{background:#eef2f7 !important;color:#333;font-weight:700;text-align:center;line-height:1.15;vertical-align:middle;}' +
                    '.sheet-table{width:100%;border-collapse:collapse;margin-bottom:18px;margin-top:0 !important;}' +
                    '.sheet-table th, .sheet-table td{border:1px solid #d1d9e6;padding:6px 8px;font-size:11px;color:#000;}' +
                    '.sheet-table thead th{background:#eef2f7 !important;color:#333;font-weight:700;}' +
                    '.share-sheet{width:100%;border-collapse:collapse;background:#fff;margin-bottom:12px;}' +
                    '.share-sheet th, .share-sheet td{border:1px solid #d1d9e6;padding:8px 10px;font-size:11px;background:#fff;word-wrap:break-word;color:#000;}' +
                    ...
                    '.share-grid{display:flex;justify-content:space-between;gap:12px;margin-bottom:14px;width:100%;}' +
                    '.share-grid>div{flex:1;min-width:0;}' +
                    
                    '/* Hide interactive controls */' +
                    '.dataTables_length, .dataTables_filter, .dataTables_info, .dataTables_paginate, .scrap-edit-btn, .scrap-edit-wrap, .no-print {' +
                    '    display:none !important;' +
                    '}' +
                    '</style></head><body><div class="export-container"><h2 style="font-family:Arial,sans-serif;margin:0 0 14px;color:#0056d0;text-align:center;text-transform:uppercase;">ARTICLE REPORT</h2>' + exportSections.join('') + '</div></body></html>';
            }
            }
