<?php include('header.php'); ?>

<style type="text/css">
    .error {
        color: red;
        float: left;
    }

    .flex_wrap {
        display: flex;
        flex-wrap: wrap;
    }

    .select2-container {
        width: 100% !important;
    }

    .right_col .page_title,
    .right_col .page_body {
        padding: -1px 8px;
    }

    .page_sec {
        border: 1px solid #ccc;
        border-radius: 5px;
        padding: 20px;
        margin-bottom: 20px;
        height: auto;
    }

    .inline-btns {
        display: flex;
        align-items: baseline;
    }


    .select2-container {
        width: 100% !important;
    }

    .modelclass {
        max-width: 60%;
        width: auto;
    }

    .content_body {
        padding: 20px;
        text-align: center;
    }
</style>
<!-- page content -->
<div class="right_col" role="main">

    <div class="table">
        <div class="page-title">
            <div class="title_left">
                <h3>Raw Material Reorder Level</h3>
            </div>

        </div>
        <div class="clearfix"></div>
        <div class="page_body">
            <div class="page_sec">
                <form method="get" name="maintenance_list" id="maintenance_list" enctype="multipart/form-data">
                    <div class="row flex_wrap">
                        <div class="form-group col-xl-3 col-lg-4 col-md-6 col-sm-12 col-xs-12">
                            <label for="plant">Plant</label>
                            <select class="form-control js-example-basic" name="plant" id="plant">
                                <option value="" selected disabled>Select Plant</option>
                                <?php if (!empty($plant)) : ?>
                                    <?php foreach ($plant as $plant_result) : ?>
                                        <option value="<?= $plant_result->id ?>" 
                                                <?= (isset($_GET['plant']) && $_GET['plant'] == $plant_result->id) ? 'selected' : '' ?>>
                                            <?= $plant_result->plant_name ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>

                        <div class="form-group col-xl-3 col-lg-4 col-md-6 col-sm-12 col-xs-12">
                            <label for="raw_material">Raw Material</label>
                            <select class="form-control js-example-basic" name="raw_material" id="raw_material">
                                <option value="" selected disabled>Select Raw Material</option>
                                <?php if (!empty($raw_material)) : ?>
                                    <?php foreach ($raw_material as $raw_material_result) : ?>
                                        <option value="<?= $raw_material_result->id ?>" 
                                                <?= (isset($_GET['raw_material']) && $_GET['raw_material'] == $raw_material_result->id) ? 'selected' : '' ?>>
                                            <?= $raw_material_result->rm_name ?>
                                        </option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                 
                    <div class="form-group col-md-12 col-sm-6 col-xs-12 mt-3 inline-btns ">
                        <button id="submit" type="submit" class="btn btn-sm btn-primary">Search</button>
                        <a href="<?= base_url() ?>raw_material_reorder_level" class="btn btn-sm btn-danger" id="reset_btn">Reset</a>
                    </div>
                    </div>
                </form>
            </div>
            <div class="x_panel">
                <div class="x_content class_style">
                    <div class="container">
                        <table style="width: 100%;" class="table table-striped table-bordered" id="example">
                            <thead class="thead">
                                <tr>
                                    <th>SR. NO.</th>
                                    <th>Plant Name</th>
                                    <th>Raw Material</th>
                                    <th>UOM</th>
                                    <th>Re-Order Level</th>
                                    <th>Stock Qty</th>
                                </tr>
                            </thead>
                        </table>
                    </div>
                </div>
            </div>
        </div>
    </div>
</div>
<input type="hidden" name="raw_material_id" id="raw_material_id" value="<?php if (isset($_GET['raw_material'])) {
    echo $_GET['raw_material'];
} ?>">

<input type="hidden" name="plant_id" id="plant_id" value="<?php if (isset($_GET['plant'])) {
    echo $_GET['plant'];
} ?>">

<?php include('footer.php'); ?>

<script>
    $(document).ready(function () {
        $('#raw_material_reorder_level').addClass('nv active-color');
        var table = $('#example').DataTable({
            'searching': true,
            "processing": true,
            "serverSide": true,
            "cache": false,
            dom: "Blfrtip",
            buttons: [
                {
                    extend: 'excel',
                    title: 'Raw Material Re-order List',
                    footer: true,
                    filename: 'raw_material_reorder_list',
                    exportOptions: {
                        columns: [0, 1, 2, 3, 4, 5]
                    }
                }
            ],
            ordering: false,
            "ajax": {
                "url": "<?= base_url() ?>admin/Ajax_controller/get_all_raw_material_reorder_level",
                "type": "POST",
                "data": function (data) {
                    data.raw_material_id = $('#raw_material_id').val();
                    data.plant_id = $('#plant_id').val();
                },
            },
        });
    });

</script>