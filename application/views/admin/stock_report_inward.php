...
<div class="right_col">
    ...
    <div class="main_page">
        ...
        <div class="page_body">
            <div class="page_sec">
                <form method="get" name="maintenance_list" id="maintenance_list" enctype="multipart/form-data">
                    <div class="row flex_wrap">
                        ...
                        <div class="form-group col-xl-3 col-lg-4 col-md-6 col-sm-12 col-xs-12">
                            <label for="type">Inward Type</label>
                        ...
                        <div class="form-group col-xl-3 col-lg-4 col-md-6 col-sm-12 col-xs-12 type-group"
                            ...
                            <label for="article">Master Batch (Color)</label>
                            <select class="form-control js-example-basic-multiple" name="color" id="color">
                                <option value="" selected disabled>Select master batch</option>
                                <?php if (!empty($color)) : ?>
                                <?php foreach ($color as $color_result) : ?>
                                <option value="<?= $color_result->id ?>"
                                    <?= (isset($_GET['color']) && $_GET['color'] == $color_result->id) ? 'selected' : '' ?>>
                                    <?= $color_result->name ?>
                                </option>
                                <?php endforeach; ?>
                                <?php endif; ?>
                    ...
                    <div class="form-group col-md-12 col-sm-6 col-xs-12 mt-3 inline-btns ">
                        ...
                        <a href="<?= base_url() ?>stock_report_inward" class="btn btn-sm btn-danger"
            ...
            <h2 class="article_group">Master Batch (Color) Report</h2>
...
<input type="hidden" name="color_id" id="color_id" value="<?php if (isset($_GET['color'])) {
    echo $_GET['color'];
} ?>">
...
$(document).ready(function() {
    ...
    $('.stock_report_inward').addClass('active_cc');
});
</script>
...
$(document).ready(function() {
    var table = $('#example').DataTable({
        ...
        "ajax": {
            "url": "<?= base_url() ?>admin/Ajax_controller/get_all_stock_report_inward_list",
            "type": "POST",
            "data": function(data) {
    ...
    var table = $('#article_list').DataTable({
        ...
        buttons: [{
            extend: 'excel',
            footer: true,
            title: 'Inward Report List',
            filename: 'Inward Report List',
            exportOptions: {
        ...
        "ajax": {
            "url": "<?= base_url() ?>admin/Ajax_controller/get_all_master_batch_stock_inward_report",
            "type": "POST",
            "data": function(data) {
                data.search_date = $('#search_date').val();
                data.plant_id = $('#search_plant_id').val();
                data.master_batch_id = $('#color_id').val();
                // data.supplier_name = $('#search_supplier_name').val();
            },
        },
